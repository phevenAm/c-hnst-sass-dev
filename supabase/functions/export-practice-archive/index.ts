...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Gather ────────────────────────────────────────────────────────────
    const [clientsRes, stubsRes, sessionsRes, stubSessionsRes, notesRes, paymentsRes, activityRes, practiceRes] =
      await Promise.all([
        supabase
          .from("users")
          .select(
        ...
        // Activity log — the same rows the /admin/audit-logs page shows this
        // admin (RLS already scopes audit_logs to actor_id = auth.uid(), but the
        // service-role client bypasses RLS so filter explicitly).
        supabase
          .from("audit_logs")
          .select("created_at, action, table_name, record_id, old_data, new_data")
          .eq("actor_id", adminId)
    ...
    for (const r of [clientsRes, stubsRes, sessionsRes, stubSessionsRes, notesRes, paymentsRes, activityRes]) {
