...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { stub_session_id, previous_date } = await req.json();
    if (!stub_session_id) {
    ...
    const newDateStr = formatDate(ss.scheduled_at);
    ...
    const html = emailTemplate({
      ...
      footerNote: "You received this email because your session was rescheduled.",
      counsellorName,
    });
